...
def resolve_persist_behavior(is_global: bool, is_session: bool) -> bool:
    ...
    3. Otherwise defer to ``model.persist_switch_by_default`` in
       ``config.yaml`` (defaults to ``False`` per Nicha's policy: a plain
       ``/model <name>`` is SESSION-SCOPED and does NOT clobber the global
       default. Use ``/model --global`` to change the standing default).
    ...
    (``False`` = session-scoped) applies.

    LOCAL-PATCH INVARIANT (registered in scripts/hermes_integrity_guard.py):
    the built-in default MUST stay ``False``. Upstream ships ``True``; a
    Hermes update that reverts this silently re-globalizes /model switches.
    """
    if is_session:
        return False
    if is_global:
